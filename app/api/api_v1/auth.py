...
from app.usable import verify_password
...
def registerUser(user_data: schemas.UserBase, db: Session = Depends(get_db)):
    try:
        existing_user = usablecrud.get_user_by_email(db, email=user_data.email)
        if existing_user:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email already registered")

        new_user = usablecrud.create_user(db, user_data)
        return new_user
...
def login_user(login_data: schemas.UserBase, db: Session = Depends(get_db)):
    try:
        user = usablecrud.get_user_by_email(db, email=login_data.email)
